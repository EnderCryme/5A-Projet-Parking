...
# ==========================================
# 1. GESTION DE LA BASE DE DONNÉES (HISTORIQUE + RFID)
# ==========================================
class ParkingDatabase:
    ...
    # --- MÉTHODES POUR LE PARKING (VIDEO) ---
    def gestion_entree_sortie(self, plaque, zone):
        """Gère l'entrée/sortie des véhicules"""
        res = "Erreur"
        try:
            with self.connect() as conn:
                c = conn.cursor()
            ...
            print(f"Err SQL Parking: {e}")
            return "Err SQL"

    # --- MÉTHODES POUR LE MQTT (RFID) ---
    # Ces méthodes sont appelées automatiquement par ton MqttManager
    def verifier_badge(self, uid):
        try:
            with self.connect() as conn:
                c = conn.cursor()
                c.execute("SELECT nom FROM badges WHERE uid = ?", (uid,))
                result = c.fetchone()
                if result:
                    return result[0] # Retourne le nom (ex: "Utilisateur 1")
                return None
        except: return None
    ...
    def creer_badge_rapide(self, uid):
        try:
            with self.connect() as conn:
                # On crée un nom par défaut
                nom_defaut = f"USER_{uid[-4:]}" 
                conn.execute("INSERT INTO badges (uid, nom) VALUES (?, ?)", (uid, nom_defaut))
                conn.commit()
                return True
        except: return False
    ...
    def supprimer_par_badge(self, uid):
        try:
            with self.connect() as conn:
                c = conn.cursor()
                c.execute("DELETE FROM badges WHERE uid = ?", (uid,))
                conn.commit()
                return c.rowcount > 0
        except: return False
...
sensor = None
mqtt = None
db = None # Notre instance de base de données
lcd_lock = threading.Lock()

# --- INIT MATERIEL ---
print("--- INIT MATERIEL ---")
try:
    # 1. On initialise la DB en premier
    db = ParkingDatabase(DB_PATH)
    print("✅ Base de données chargée")

    # 2. On initialise le reste
    lcd = LcdManager(cs_pin=LCD_CS)
    sensor = SensorManager(cs_pin=SENSOR_CS)
    
    # 3. ON PASSE LA DB AU MQTT MANAGER !
            ...
            if match:
                ...
                if count >= SAMPLES_TO_TAKE:
                    ...
                    if plaque_validated != current_view[zone]:
                        # 1. DB (Appel via la classe)
                        info_db = db.gestion_entree_sortie(plaque_validated, zone)
                        
                        # 2. MQTT (Video)
                        if mqtt:
                            topic = "entree" if zone == "in" else "sortie"
                            mqtt.publish(topic, f"{plaque_validated}|{info_db}")
...
# --- WEB AVEC HISTORIQUE SCROLLABLE & BADGE CORRIGÉ ---
@app.route('/')
def index():
    return """
    <!DOCTYPE html>
            ...
            <div class="row mt-4">
                <div class="col-12">
                    <div class="table-container">
                        <h4 class="mb-3"><i class="bi bi-clock-history"></i> Historique (Défilement)</h4>
