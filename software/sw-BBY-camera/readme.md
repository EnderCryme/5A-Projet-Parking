...
Ce dépôt contient le "cerveau" du système de parking intelligent. Exécuté sur une **BeagleBone Black**, ce nœud centralise la logique métier, la gestion de la base de données et le traitement d'images.
...
* **Reconnaissance LAPI (ALPR) :** Détection et lecture automatique des plaques d'immatriculation via **OpenCV** et **Tesseract OCR**.
...
* **Affichage Local (SPI) :** Pilotage d'une matrice LED/LCD pour les messages d'accueil et la météo locale (BME680).
* **Mode Hybride :** Capacité de fonctionner en mode "Simulation" sur PC (Windows) ou en mode "Matériel Réel" sur Linux.

## 🛠 Architecture Matérielle

* **SBC :** BeagleBone Black Wireless (Debian Linux)
* **Vision :** Webcams USB (Entrée/Sortie)
* **Affichage :** Matrice LED / LCD (SPI)
...
Le serveur agit comme le maître logique sur le réseau.
...
| `parking/barrier_x/state` | Pub (Out) | `OPEN` / `CLOSE` | Ordre d'ouverture des barrières. |
| `RFID/ADD` | Sub (In) | `[UID]` | Demande d'ajout rapide d'un badge (Admin). |

## Ouverture : Architecture Cloud & Edge Computing (Déploiement du site)

Actuellement, le projet fonctionne en mode "autonome" (le serveur Web et l'IA tournent sur la même machine). Pour un déploiement réel à grande échelle sur internet (ex: via Render ou AWS), l'architecture évoluerait vers un modèle **IoT Edge** :
...
2. **Sur le Cloud (Server Node)** : L'application Flask (`main.py`) est hébergée sur un serveur distant. Elle gère :
* La base de données centralisée (PostgreSQL).
...
Le projet nécessite Python 3.x et les drivers système.

### Pré-requis

```bash
# Installation des dépendances système (Debian/Ubuntu)
sudo apt-get install python3-opencv tesseract-ocr libopenjp2-7
