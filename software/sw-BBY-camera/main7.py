            ...
            if re.search(r"([A-Z]{2})-?([0-9]{3})-?([A-Z]{2})", corr):
                ...
                if len(vote_buffers[zone]) < SAMPLES_TO_TAKE:
                    ...
                    if most_common != current_view[zone]:
                        ...
                        # --- MODIFICATION DEMANDÉE : OUVERTURE BARRIÈRE SORTIE ---
                        # Si c'est la zone OUT et qu'on a trouvé la plaque, on ouvre la barrière
                        if zone == "out" and mqtt:
                            mqtt.publish("barrier/sortie", "OPEN")
                            print("🚀 OUVERTURE BARRIÈRE SORTIE")
                        # ---------------------------------------------------------
